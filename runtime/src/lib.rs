...
mod test {
    use super::*;
    use frame::{runtime::types_common::AccountId, testing_prelude::*};
    use pallet_minimal_template::*;

    const ALICE: <Runtime as frame_system::Config>::AccountId = AccountId::new([1u8; 32]);
    const BOB: <Runtime as frame_system::Config>::AccountId = AccountId::new([2u8; 32]);
    const CHARLIE: <Runtime as frame_system::Config>::AccountId = AccountId::new([3u8; 32]);
        ...
        pub(crate) fn build_and_execute(self, test: impl FnOnce() -> ()) {
            ...
            ext.execute_with(|| {
                for (who, amount) in &self.balances {
                    pallet_minimal_template::Balances::<Runtime>::insert(who, amount);
            ...
            ext.execute_with(|| {
                assert_eq!(
                    pallet_minimal_template::Balances::<Runtime>::iter()
                        .map(|(_, x)| x)
                        .sum::<u128>(),
    ...
    fn state_builder_works() {
        StateBuilder::default().build_and_execute(|| {
            assert_eq!(
                pallet_minimal_template::Balances::<Runtime>::get(&ALICE),
                Some(100)
            );
            assert_eq!(
                pallet_minimal_template::Balances::<Runtime>::get(&BOB),
                Some(100)
            );
            assert_eq!(
                pallet_minimal_template::Balances::<Runtime>::get(&CHARLIE),
                None
            );
    ...
    fn transfer_exceed_balance_fails() {
        StateBuilder::default().build_and_execute(|| {
            ...
            assert_err!(
                Pallet::<Runtime>::transfer(RuntimeOrigin::signed(ALICE), BOB, 101),
